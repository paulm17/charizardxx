...
export type CharizardxxStyleProp = CharizardxxStyle | CharizardxxStyle[] | CharizardxxStyleProp[] | undefined;
export type CssVariable = `--${string}`;
export type CssVariables<Variable extends string = CssVariable> = Partial<Record<Variable, string>>;
export type CssVars<Variable extends string = CssVariable> = CssVariables<Variable> | ((theme: CharizardxxTheme) => CssVariables<Variable>) | CssVars<Variable>[];
