...
`@charizardxx/form` schema validation resolver packages are now available as [separate packages](https://mantine.dev/form/schema-validation).
Moving resolvers to separate packages allows making them type-safe and fully tested.
Old resolvers are still exported from `@charizardxx/form` package – you will be able to use them without any changes
until 8.0.0 release.
...
both in `rem` function exported from `@charizardxx/core` package and [postcss-preset-mantine](https://mantine.dev/styles/postcss-preset).
...
```tsx
import { Modal } from '@charizardxx/core';
...
```tsx
import { Group } from '@charizardxx/core';
...
```tsx
import { Checkbox, Radio, Stack } from '@charizardxx/core';
...
import { HueSlider, Text } from '@charizardxx/core';
...
```tsx
import { useState } from 'react';
import { AlphaSlider, Text } from '@charizardxx/core';
...
```tsx
import { Button, Drawer } from '@charizardxx/core';
...
- Schema-based validation with `@charizardxx/form` now has a [dedicated page](https://mantine.dev/form/schema-validation). It includes more examples for basic, nested and list fields validation for each resolver.
...
- [mantine-flagpack](https://mantinedev.github.io/mantine-flagpack/) has been migrated to support `7.x` versions of `@charizardxx/core`. To use it, update `mantine-flagpack` to `4.0.0`.
...
- `@charizardxx/ds` package has been deprecated. You can use `@charizardxxx/mantine-logo` package to use `MantineLogo` component in your project.
