// Generates separate css files for each @charizardxx/core component
...
// Generates styles.layers.css files for each @charizardxx/* component
...
// Generates individual css files for each @charizardxx/core component
export async function generateCoreCSS() {
  const packagesPath = glob.convertPathToPattern(getPath('packages'));
  const files = await glob(`${packagesPath}/@charizardxx/core/src/**/*.css`);
  ...
  const outputFolder = getPath('packages/@charizardxx/core/styles');
