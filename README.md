...
2.  There may be design decisions implemented that will diverge from Mantine.
    ...
    - Mantine and Charizardxx employ different approaches to manage the theme. While
      Mantine uses context and can be a runtime process for overrides. Charizardxx
...
```sh
yarn add @charizardxx/core @charizardxx/emotion @charizardxx/system
yarn add -D @stylefusion/vite-plugin
```

Note: Charizardxx uses yarn workspaces as there are various issues with pnpm.
...
import { getPigmentCSSTheme } from '@charizardxx/system';
...
```sh
git clone https://github.com/paulm17/charizardxx
```
...
- [ ] Ensure Mantine tests pass with Charizardxx.
